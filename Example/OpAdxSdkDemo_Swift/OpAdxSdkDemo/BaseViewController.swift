...
class BaseViewController: UIViewController {
    ...
    private var videoMutedLabel = UILabel()
    private var videoMutedSegmentedControl = UISegmentedControl()
    ...
    private func setupUI() {
        ...
        // 只有非banner格式才显示Video Muted控件
        if adFormat != .native {
            videoMutedLabel.text = "Video Muted:"
            videoMutedSegmentedControl.insertSegment(withTitle: "Default", at: 0, animated: false)
            videoMutedSegmentedControl.insertSegment(withTitle: "Yes", at: 1, animated: false)
            videoMutedSegmentedControl.insertSegment(withTitle: "No", at: 2, animated: false)
            if let mute = OpAdxSdkCore.getOpAdxSdkMuted()  {
                if mute.boolValue {
                    videoMutedSegmentedControl.selectedSegmentIndex = 1
                }else {
                    videoMutedSegmentedControl.selectedSegmentIndex = 2
                }
            } else {
                videoMutedSegmentedControl.selectedSegmentIndex = 0
            }
            videoMutedSegmentedControl.addTarget(self, action: #selector(videoMutedChanged(_:)), for: .valueChanged)
        ...
        // 添加基本控件
        [adFormatLabel, placementIdLabel, loadAdButton, showAdButton, destroyAdButton, adContainer].forEach {
            view.addSubview($0)
        ...
        if adFormat != .native {
            view.addSubview(videoMutedLabel)
            view.addSubview(videoMutedSegmentedControl)
            videoMutedLabel.translatesAutoresizingMaskIntoConstraints = false
            videoMutedSegmentedControl.translatesAutoresizingMaskIntoConstraints = false
        }
        
        logView.setup(in: view)
    }
    
    @objc private func videoMutedChanged(_ sender: UISegmentedControl) {
        ...
        case 1: // Yes
            OpAdxSdkCore.setOpAdxSdkMuted(NSNumber(value: true))
        case 2: // No
            OpAdxSdkCore.setOpAdxSdkMuted(NSNumber(value: false))
        default:
            OpAdxSdkCore.setOpAdxSdkMuted(nil)
        }
    }
    private func setupConstraints() {
        if adFormat == .native {
            ...
            NSLayoutConstraint.activate([
                ...
                videoMutedLabel.topAnchor.constraint(equalTo: placementIdLabel.bottomAnchor, constant: 20),
                videoMutedLabel.leadingAnchor.constraint(equalTo: view.leadingAnchor, constant: 20),
                
                videoMutedSegmentedControl.centerYAnchor.constraint(equalTo: videoMutedLabel.centerYAnchor),
                videoMutedSegmentedControl.leadingAnchor.constraint(equalTo: videoMutedLabel.trailingAnchor, constant: 10),
                videoMutedSegmentedControl.widthAnchor.constraint(equalToConstant: 200),
                
                loadAdButton.topAnchor.constraint(equalTo: videoMutedLabel.bottomAnchor, constant: 20),
                loadAdButton.leadingAnchor.constraint(equalTo: view.leadingAnchor, constant: 20),
                loadAdButton.widthAnchor.constraint(equalToConstant: 100),
                loadAdButton.heightAnchor.constraint(equalToConstant: 44),
